...
const delayBR = 80;
const inputFile = 'cars_misc_rules.txt';
const outputFile = 'cars_misc_output.txt';
...
async function checkURL(url) {
    try {
        ...
        if (response.status === 403) {
            console.log(`403 Forbidden (considered valid): ${url}`);
            return true;
        } else if (!response.ok) {
        ...
        return true; // Consider fetch errors as valid
    }
}
